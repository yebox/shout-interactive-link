const Text = ({ placeholder = "placeholder", label = "", status = "", message = "" }) => {
  return (
    <div>
      <label className="caption_heavy text-black-default flex mb-[8px]">{label}</label>
      <input
        className={`flex h-[48px] mb-[10px] max-w-[375px] min-w-[250px] w-full text-black-default body_light focus:border-primary focus:outline-0 ${
